...
![image-20250510112611674](../../AppData/Roaming/Typora/typora-user-images/image-20250510112611674.png)
...
<body>
    ...
    <script>
        function tdate() {
            ...
            // 在页面上显示
            document.getElementById("update").textContent = day
...
- **事件冒泡**：事件从最具体的元素（触发事件的元素）开始，然后向外传播到其父元素，直到传播至文档根节点。就像是把石头扔进水里，波纹是从中心向外扩散。

  * 概念：当一个事件发生时，它首先在最具体的元素（即事件目标）上触发，然后逐级向外传播至它的父元素、祖父元素等，直到传播到整个DOM树的根节点（通常是`document`或`window`对象）。这种现象被称为事件冒泡。
  ...
  <body>
      ...
      <script>
          ...
          const effervescenceDiv = document.getElementById('effervescence');
          ...
          // 添加事件监听器到<p>标签上，默认冒泡阶段处理
          secondP.addEventListener('click', function () {
              console.log('second clicked');
          });
  
          // 添加事件监听器到<div>标签上，默认冒泡阶段处理
          effervescenceDiv.addEventListener('click', function () {
              console.log('effervescence clicked');
          });
      </script>
  </body>
...
1. **简化事件处理**：通过在高层级的父元素上监听事件，可以减少为每个子元素单独添加事件监听器的需求。这特别适用于具有许多动态添加或删除的子元素的情况，如列表项。
2. **提高性能**：减少直接绑定事件处理器到大量元素的做法，改为在共同的父元素上监听和处理事件，能够降低内存使用并提升性能。
3. **全局事件处理**：可以在文档的最高层监听所有类型的事件，然后根据事件的目标元素来决定如何处理这些事件。这种方式非常适合于实现全局键盘快捷键、统一的错误处理机制等。
4. **事件委托**：利用事件冒泡特性，将事件处理逻辑委托给父元素处理，即使新添加的子元素也能够响应事件，无需对新增元素重新绑定事件监听器。
...
1. **优先级处理**：通过在捕获阶段添加事件监听器，可以在其他大多数事件监听器之前处理特定事件。这对于需要首先执行的逻辑特别有用，比如阻止某些默认行为或立即响应用户交互而不等待事件传播到目标元素。
2. **全局事件拦截**：如果你想在整个页面范围内对某些类型的事件进行统一管理或限制（例如，在整个网站上禁止右键菜单），可以通过在文档的根元素上使用事件捕获来实现，这样可以确保这些规则在事件到达任何具体的目标元素之前得到应用。
3. **防止子元素的不必要触发**：有时你可能希望在父元素级别处理某个事件，而不想让这个事件传递给它的子元素。利用事件捕获阶段的监听器，可以在事件到达那些子元素之前就进行处理并停止其进一步传播，避免不必要的事件触发。
4. **增强用户体验**：在一些复杂的用户界面中，合理运用事件捕获能够帮助创建更流畅的用户体验。例如，在一个多层嵌套的菜单系统中，可以使用事件捕获来快速关闭所有打开的菜单项，而不是等待事件冒泡至每个具体的菜单项去处理关闭逻辑。
5. **兼容性和灵活性**：理解并使用事件捕获增加了前端开发者的工具箱，使得他们可以根据需要选择最合适的事件处理策略。无论是为了优化性能、改善用户交互体验还是解决特定的技术挑战，灵活运用事件捕获都能提供额外的解决方案。
